...
public class CoapApplServer extends CoapServer { //implements IContext
	...
	public CoapApplServer( int port ) {
		super(port);
		add( root );
		 CommUtilsOrig.outyellow("    +++ CoapApplServer | STARTED root=" + root + " port="+port );
		server = this;
		start();
	}
	...
	public static Resource getResource( String uri ) {
		CommUtilsOrig.outblue("    +++ CoapApplServer | getResource uri="  + uri + " root=" + root.getName());
	...
	private static Resource getResource(Resource root, String uri) {
		if( root != null ) {
			CommUtilsOrig.outyellow("    +++ CoapApplServer | getResource checks in: " + root.getName() + " for uri=" + uri);
			...
			while( iter.hasNext() ) {
				...
				CommUtilsOrig.outyellow("    +++ CoapApplServer | getResource curUri:"+curUri);
				if( curUri.equals(uri) ){
					CommUtilsOrig.outyellow("    +++ CoapApplServer | getResource finds "+ curRes.getName() + " for " + curUri);
					return  curRes;
				}else { 
	...
	public  void addCoapResource( CoapResource resource, String fatherUri  )   {
		CommUtilsOrig.outblue("    +++ CoapApplServer | addCoapResource resource=" + resource.getName() + " fatherUri=" + fatherUri );
