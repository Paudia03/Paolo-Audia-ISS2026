...
public UdpApplMessageHandler(  IApplMsgHandler handler, Interaction  conn ) {
		this.handler = handler;
		this.conn    = conn;
		CommUtilsOrig.outyellow("    +++ UdpApplMessageHandler | CREATED for conn=" + conn   ); 
 		this.start();
	}
	...
	public void run() {
		...
		try {
			CommUtilsOrig.outyellow( "    +++ UdpApplMessageHandler | STARTS with handler=" + name + " conn=" + conn  );
			while( true ) {
			...
			CommUtilsOrig.outred( "    +++ UdpApplMessageHandler  | ERROR:" + e.getMessage()  );
		}	
	}
}
