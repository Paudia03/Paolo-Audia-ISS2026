...
protected boolean isactive = false;
	...
	public void addMsgHandler(IApplMsgHandler handler)  {
		...
		}else CommUtilsOrig.outred(name+" | EnablerAsServer CREATED no protocol"  );
	}
	//Crea il server
 	protected void setServerSupport( int port, ProtocolType protocol, IApplMsgHandler handler   )  {
      try {
		if( protocol == ProtocolType.tcp  ) {
			...
			CommUtilsOrig.outblue(name+" |  TODO CREATED  CoapApplServer"  );
		}
		else if( protocol == ProtocolType.mqtt ) {  
			CommUtilsOrig.outblue(name+" |  Do nothing for mqtt" );
		}
     } catch (Exception e) {
		CommUtilsOrig.outred(name+" |  EnablerAsServer CREATE Error: " + e.getMessage()  );
     }
	}
