...
protected int port;
	...
	public void run() {
	      try {
			...
			while( ! stopped ) {
				//Accept a connection				 
				//CommUtils.outyellow(getName() + " | TcpServer waiting on port " + port );
		 		Socket sock          = serversock.accept();
				CommUtilsOrig.outyellow(getName()
			  ...
			  CommUtilsOrig.outred(getName() + " | probably socket closed: " + e.getMessage() );
		  }
	}
	...
	public void deactivate() {
		try {
			CommUtilsOrig.outyellow(getName()+" |  DEACTIVATE serversock=" +  serversock);
			stopped = true;
            serversock.close();
		} catch (Exception e) {
			CommUtilsOrig.outred(getName() + " | deactivate ERROR: " + e.getMessage());
		}
	}
